...
class BudgetTracker:
    ...
    def __init__(self):
        self._monthly_budget: float = float(os.getenv("OPENAI_MONTHLY_BUDGET", "5.00"))
        self._daily_budget: float = float(os.getenv("OPENAI_DAILY_BUDGET", "0.50"))
        self._max_requests_per_hour: int = int(os.getenv("OPENAI_MAX_REQUESTS_PER_HOUR", "5"))

        self._usage_file = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "usage.json")
    ...
    def can_make_request(self) -> tuple[bool, str]:
        ...
        if self._usage["monthly_cost"] >= self._monthly_budget:
            msg = f"Monthly budget exhausted: ${self._usage['monthly_cost']:.4f} / ${self._monthly_budget:.2f}"
            logger.warning(msg)
            return False, msg

        # Check daily budget
        if self._usage["daily_cost"] >= self._daily_budget:
            msg = f"Daily budget exhausted: ${self._usage['daily_cost']:.4f} / ${self._daily_budget:.2f}"
            logger.warning(msg)
            return False, msg
        ...
        if self._usage["hourly_requests"] >= self._max_requests_per_hour:
            msg = (
                f"Hourly rate limit reached: {self._usage['hourly_requests']} / {self._max_requests_per_hour} requests"
            )
            logger.warning(msg)
            return False, msg
    ...
    def record_usage(self, input_tokens: int, output_tokens: int):
        ...
        self._usage["last_request"] = datetime.now(UTC).isoformat()
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "remaining": {
                "monthly_budget": round(self._monthly_budget - self._usage["monthly_cost"], 6),
                "daily_budget": round(self._daily_budget - self._usage["daily_cost"], 6),
            },
